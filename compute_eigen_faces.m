...
% adapted from webpage http://www.pages.drexel.edu/~sis26/Eigencode.htm
...
data_folder = 'labelled_dataset/';
output_folder = 'averaged_images/';
...
for i=1:M
    ...
    if mod(i,showim) == 0
        imshow(img)
        title(sprintf('Training set %i',i),'fontsize',18)
    end
    drawnow;
    if size(size(img)) == [1 3]
        img = uint8(floor(mean(img,3)));
    end
    img = img(16:415,:);
    if size(img,2) == 720
        img =img(:,41:680);
...
Means_images = zeros(1,M);
Std_images = zeros(1,M);
...
figure(2);
for i=1:M
    str=strcat(output_folder,int2str(i),'.png');
...
%     imshow(img)
    if mod(i,showim) == 0
        imshow(img)
        title(sprintf('Normalized Training Set %i',i),'fontsize',18)
    end
    drawnow;
end
...
weights_file_name = 'PCA_weights.txt';
